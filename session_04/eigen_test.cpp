...
main ()
{
  ...
  ofstream output;
  output.open("eigen.dat");

  for(int i= 2; i <= dimension; i++)
  {

  // allocate space for the vectors, matrices, and workspace 
  Amat_ptr = gsl_matrix_alloc (i, i);
  Eigval_ptr = gsl_vector_alloc (i);
  Eigvec_ptr = gsl_matrix_alloc (i, i);
  worksp = gsl_eigen_symmv_alloc (i);
  eigenvector_ptr = gsl_vector_alloc (i);
  eigenvalue = 0;
  ...
  for (int k = 0; k < i; k++)
    {
      for (int j = 0; j < i; j++)
	{
	  hilbert = 1. / ((float) (k + j + 1));	// k,j start at 0 
	  gsl_matrix_set (Amat_ptr, k, j, hilbert);
	}
    }
