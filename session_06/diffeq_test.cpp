...
main (void)
{
  ...
  double hmin = .00001;		// starting t value 
  double hmax = 0.01;		// last t value 

  // print out a header line and the first set of points 
  out << "#      h           y_euler(t)         y_rk4(t)        y_exact(t) \n";


  for (double h = hmin; h <= hmax; h += hmin)
  {
    double t = 1.0;
    ...
    out << scientific << setprecision (16)
      << h << "  "
      << y_euler[0] << "  "<< abs(y_euler[0] - exact_answer (t + h, params_ptr))/exact_answer (t + h, params_ptr)<< "   "
      << y_rk4[0] << "  " << abs(y_rk4[0]-exact_answer (t + h, params_ptr))/exact_answer (t + h, params_ptr)<<
	"  "<<exact_answer (t + h, params_ptr) << endl;
  }
